...
function ExpForm({ id, onClose }) {
  ...
  const onSubmit = (e) => {
    e.preventDefault();
    // console.log({ id,desc, cost, tag, mode, onlyDate })
    dispatch(addExpense({ id, desc, cost, tag, mode, onlyDate }));
    setDesc("");
    setCost(0);
    setTag("");
    setMode("");
  ...
  function Dropdown({ selectedOption, isDropdownOpen, handleOptionSelect }) {
    ...
    return (
      <div className="w-56">
        ...
        {isDropdownOpen && (
          <ul className="absolute mt-2 text-gray-700 hover:text-black py-1 w-fit bg-white rounded-[10px] shadow-lg ">
            {options.map((option) => (
